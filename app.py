...
# ==============================================================================
# 🔴 CẤU HÌNH DỮ LIỆU (TÔI ĐÃ ĐIỀN SẴN LINK CHUẨN TỪ ẢNH CỦA BẠN)
# ==============================================================================

# 1. Link CSV Công Việc (ĐÃ CHUẨN)
...
# 2. Link Google Calendar (ĐÃ CHUẨN)
...
if df_congviec is None:
    st.error("⚠️ Chưa đọc được dữ liệu. Hãy kiểm tra xem file Google Sheet có đang 'Xuất bản' (Published) không.")
    st.stop()
...
with tab1:
    ...
    if not df_loc.empty:
        ...
        now = datetime.now()
        k1.metric("Tổng việc", len(df_loc))
        k2.metric("Đã xong", len(df_loc[df_loc["Trạng Thái"].str.contains("Hoàn", na=False)]))
        k3.metric("🚨 Quá hạn", len(df_loc[(~df_loc["Trạng Thái"].str.contains("Hoàn", na=False)) & (df_loc["Hạn Chót"] < now)]))
...
with tab2:
    ...
    if "http" in LINK_GOOGLE_CALENDAR:
        # Tự động chuyển sang chế độ Lịch Biểu (AGENDA) để gọn gàng
